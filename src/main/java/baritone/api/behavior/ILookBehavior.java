...
public interface ILookBehavior extends IBehavior {
	...
	/**
	 * Updates the current {@link ILookBehavior} target to target the specified rotations on the next tick. If force is {@code true}, then freeLook will be overriden and angles will be set regardless. If any sort of block interaction is required, force should be {@code true}, otherwise, it should be {@code false};
	 *
