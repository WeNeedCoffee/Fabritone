...
public final class Settings {
	...
	/**
	 * Renders X/Z type Goals with the vanilla beacon beam effect. Combining this with {@link #renderGoalIgnoreDepth} will cause strange render clipping.
	 */
	...
	/**
	 * When GetToBlockProcess or MineProcess fails to calculate a path, instead of just giving up, mark the closest instance of that block as "unreachable" and go towards the next closest. GetToBlock expands this seaarch to the whole "vein"; MineProcess does not. This is because MineProcess finds individual impossible blocks (like one block in a vein that has gravel on top then lava, so it can't break) Whereas GetToBlock should blacklist the whole "vein" if it can't get to any of them.
	 */
	...
	/**
	 * Disallow MineBehavior from using X-Ray to see where the ores are. Turn this option on to force it to mine "legit" where it will only mine an ore once it can actually see it, so it won't do or know anything that a normal player couldn't. If you don't want it to look like you're X-Raying, turn this on
	 */
	...
	/**
	 * The function that is called when Baritone will log to chat. This function can be added to via {@link Consumer#andThen(Consumer)} or it can completely be overriden via setting {@link Setting#value};
	 */
