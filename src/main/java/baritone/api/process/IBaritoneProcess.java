...
public interface IBaritoneProcess {
	...
	/**
	 * Called if {@link #isActive} returned {@code true}, but another non-temporary process has control. Effectively the same as cancel. You want control but you don't get it.
	 */
	void onLostControl();
	 ...
	 *
	 * @param calcFailed     {@code true} if this specific process was in control last tick, and there was a {@link PathEvent#CALC_FAILED} event last tick
	 * @param isSafeToCancel {@code true} if a {@link PathingCommandType#REQUEST_PAUSE} would happen this tick, and {@link IPathingBehavior} wouldn't actually tick. {@code false} if the PathExecutor reported pausing would be unsafe at the end of the last tick. Effectively "could request cancel or pause and have it happen right away"
	 * @return What the {@link IPathingBehavior} should do
	 */
	PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel);

	/**
	 * Used to determine which Process gains control if multiple are reporting {@link #isActive()}. The one that returns the highest value will be given control.
	 *
