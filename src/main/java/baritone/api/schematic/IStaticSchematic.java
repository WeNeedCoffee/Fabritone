...
public interface IStaticSchematic extends ISchematic {
	...
	/**
	 * Returns an {@link BlockState} array of size {@link #heightY()} which contains all desired block states in the specified vertical column. The index of {@link BlockState}s in the array are equivalent to their Y position in the schematic.
	 *
	...
	default BlockState[] getColumn(int x, int z) {
		BlockState[] column = new BlockState[heightY()];
	...
	/**
	 * Gets the {@link BlockState} for a given position in this schematic. It should be guaranteed that the return value of this method will not change given that the parameters are the same.
	 *
