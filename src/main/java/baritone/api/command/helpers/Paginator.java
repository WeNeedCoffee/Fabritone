...
public class Paginator<E> implements Helper {
	...
	public void display(Function<E, Text> transform, String commandPrefix) {
		...
		Text prevPageComponent = new LiteralText("<<");
		if (hasPrevPage) {
			prevPageComponent.getStyle().setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, String.format("%s %d", commandPrefix, page - 1))).setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new LiteralText("Click to view previous page")));
		} else {
			prevPageComponent.getStyle().setColor(Formatting.DARK_GRAY);
		}
		Text nextPageComponent = new LiteralText(">>");
		if (hasNextPage) {
			nextPageComponent.getStyle().setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, String.format("%s %d", commandPrefix, page + 1))).setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new LiteralText("Click to view next page")));
		} else {
			nextPageComponent.getStyle().setColor(Formatting.DARK_GRAY);
		}
		Text pagerComponent = new LiteralText("");
		pagerComponent.getStyle().setColor(Formatting.GRAY);
