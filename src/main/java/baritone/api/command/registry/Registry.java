...
/**
 * This registry class allows for registration and unregistration of a certain type. This is mainly designed for use by event handlers where newly registered ones are encountered first during iteration and can therefore override older ones. In Baritone, this is used for commands and argument parsers so that mods and addons can extend Baritone's functionality without resorting to hacks, wrappers, or mixins.
 *
 * @param <V> The entry type that will be stored in this registry. This can be anything, really - preferably anything that works as a HashMap key, as that's what's used to keep track of which entries are registered or not.
 */
public class Registry<V> {

	/**
	 * An internal linked list of all the entries that are currently registered. This is a linked list so that entries can be inserted at the beginning, which means that newer entries are encountered first during iteration. This is an important property of the registry that makes it more useful than a simple list, and also the reason it does not just use a map.
	 */
	private final Deque<V> _entries = new LinkedList<>();
	/**
	 * A HashSet containing every entry currently registered. Entries are added to this set when something is registered and removed from the set when they are unregistered. An entry being present in this set indicates that it is currently registered, can be removed, and should not be reregistered until it is removed.
	 */
	 ...
	 * @return A boolean indicating whether or not this is a new registration. No matter the value of this boolean, the entry is always guaranteed to now be in this registry. This boolean simply indicates if the entry was <i>not</i> in the map prior to this method call.
	 */
	...
	/**
	 * Unregisters this entry from this registry. After this method call, the entry is guaranteed to be removed from the registry, since each entry only ever appears once.
	 *
