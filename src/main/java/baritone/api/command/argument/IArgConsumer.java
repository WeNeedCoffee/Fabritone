 ...
 * <ul>
 * <li>Mutability. The whole concept of the {@link IArgConsumer}} is to let you gradually consume arguments in any way you'd like. You can change your consumption based on earlier arguments, for subcommands for example.</li>
 ...
 * <li>Easy retrieval of many different types. If you need to retrieve an instance of an int or float for example, look no further than {@link #getAs(Class)}. If you need a more powerful way of retrieving data, try out the many {@code getDatatype...} methods.</li>
 * <li>It's very easy to throw detailed exceptions. The {@link IArgConsumer}} has many different methods that can enforce the number of arguments, the type of arguments, and more, throwing different types of {@link CommandException}s if something seems off. You're recommended to do all validation and store all needed data in variables BEFORE logging any data to chat via {@link Helper#logDirect(String)}, so that the error handlers can do their job and log the error to chat.</li>
 * </ul>
 */
public interface IArgConsumer {

	/**
	 * @return The last argument this {@link IArgConsumer}} has consumed, or an "unknown" argument, indicated by a comamnd argument index that has a value of {@code -1}, if no arguments have been consumed yet.
	 * @see #consumedString()
	 * @see #hasConsumed()
	 */
	...
	/**
	 * @return The value of thelast argument this {@link IArgConsumer}} has consumed, or an empty string if no arguments have been consumed yet
	 * @see #consumed()
	 * @see #hasConsumed()
	 */
	...
	/**
	 * @return A copy of this {@link IArgConsumer}}. It has the same arguments (both consumed and not), but does not affect or mutate this instance. Useful for the various {@code peek} functions
	 */
	IArgConsumer copy();

	/**
	 * Gets the next argument and returns it. This consumes the first argument so that subsequent calls will return later arguments
	 *
	 ...
	 * <p>
	 * For example if you getEnum as an {@link Direction}, and the next argument's value is "up", this will return {@link Direction#UP}
	 *
	 ...
	 * @param enumClass The enum class to search
	 * @param def       The default value
	 * @return An enum constant of that class with the same name as the next argument's value, or {@code def} if it couldn't be found
	 * @see #getEnum(Class)
	 * @see #getEnumOrNull(Class)
	 ...
	 *
	 * @param enumClass The enum class to search
	 * @return An enum constant of that class with the same name as the next argument's value, or {@code null} if it couldn't be found
	 * @see #getEnum(Class)
	...
	/**
	 * Gets the value of the next argument and returns it. This consumes the first argument so that subsequent calls will return later arguments
	 *
	 ...
	 * @return If an ArgParser.Stateless for the specified {@code type} would succeed in parsing the next argument
	...
	/**
	 * @param index The index to peek
	 * @return The argument at index {@code index} in this {@link IArgConsumer}}, with 0 being the next one. This does not mutate the {@link IArgConsumer}}
	 * @throws CommandNotEnoughArgumentsException If there is less than {@code index + 1} arguments left
	 * @see #peek()
	 * @see #peekString(int)
	...
	/**
	 * Tries to use a <b>stateless</b> {@link IArgParser} to parse the argument at the specified index into the specified class
	 * <p>
	 * A critical difference between {@link IDatatype}s and {@link IArgParser}s is how many arguments they can take. While {@link IArgParser}s always operate on a single argument's value, {@link IDatatype}s get access to the entire {@link IArgConsumer}}.
	 *
	 * @param type  The type to peek as
	 ...
	 * @return From the specified enum class, an enum constant of that class. The enum constant's name will match the next argument's value
	 * @throws CommandInvalidTypeException If the constant couldn't be found
	 * @see #peekEnumOrNull(Class)
	 * @see #getEnum(Class)
	 * @see ICommandArgument#getEnum(Class)
	...
	/**
	 * @param enumClass The class to search
	 * @return From the specified enum class, an enum constant of that class. The enum constant's name will match the next argument's value. If no constant could be found, null
	 * @see #peekEnum(Class)
	 * @see #getEnumOrNull(Class)
	 * @see ICommandArgument#getEnum(Class)
	 */
	...
	/**
	 * @param index The index to peek
	 * @return The value of the argument at index {@code index} in this {@link IArgConsumer}}, with 0 being the next one This does not mutate the {@link IArgConsumer}}
	 * @throws CommandNotEnoughArgumentsException If there is less than {@code index + 1} arguments left
	 * @see #peek()
	 * @see #peekString()
	 */
	...
	/**
	 * Returns the "raw rest" of the string. For example, from a string <code>arg1 arg2&nbsp;&nbsp;arg3</code>, split into three {@link ICommandArgument}s {@code "arg1"}, {@code "arg2"}, and {@code "arg3"}:
	 * <ul>
	 * <li>{@code rawRest()} would return <code>arg1 arg2&nbsp;&nbsp;arg3</code></li>
	 * <li>After calling {@link #get()}, {@code rawRest()} would return <code>arg2&nbsp;&nbsp;arg3</code> (note the double space - it is preserved!)</li>
