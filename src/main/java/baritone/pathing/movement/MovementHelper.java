...
public interface MovementHelper extends ActionCosts, Helper {
	...
	static boolean avoidAdjacentBreaking(BlockStateInterface bsi, int x, int y, int z, boolean directlyAbove) {
		...
		if (!directlyAbove // it is fine to mine a block that has a falling block directly above, this (the cost of breaking the stacked fallings) is included in cost calculations
				...
				&& FallingBlock.canFallThrough(bsi.get0(x, y - 1, z)))
	...
	static boolean avoidBreaking(BlockStateInterface bsi, int x, int y, int z, BlockState state) {
		...
		return b == Blocks.ICE // ice becomes water, and water can mess up the path
				...
				|| avoidAdjacentBreaking(bsi, x, y + 1, z, true) || avoidAdjacentBreaking(bsi, x + 1, y, z, false) || avoidAdjacentBreaking(bsi, x - 1, y, z, false) || avoidAdjacentBreaking(bsi, x, y, z + 1, false) || avoidAdjacentBreaking(bsi, x, y, z - 1, false);
	}

	static boolean avoidWalkingInto(BlockState state) {
		Block block = state.getBlock();
		return !state.getFluidState().isEmpty() || block == Blocks.MAGMA_BLOCK || block == Blocks.CACTUS || block == Blocks.FIRE || block == Blocks.END_PORTAL || block == Blocks.COBWEB || block == Blocks.BUBBLE_COLUMN;
	}
	...
	/**
	 * Can I walk on this block without anything weird happening like me falling through? Includes water because we know that we automatically jump on water
	 *
	...
	static boolean fullyPassable(BlockView world, BlockPos pos, BlockState state) {
		...
		if (block == Blocks.FIRE || block == Blocks.TRIPWIRE || block == Blocks.COBWEB || block == Blocks.VINE || block == Blocks.LADDER || block == Blocks.COCOA || block instanceof DoorBlock || block instanceof FenceGateBlock || block instanceof SnowBlock || !state.getFluidState().isEmpty() || block instanceof TrapdoorBlock || block instanceof EndPortalBlock || block instanceof SkullBlock || block instanceof ShulkerBoxBlock)
			return false;
	...
	/**
	 * canWalkThrough but also won't impede movement at all. so not including doors or fence gates (we'd have to right click), not including water, and not including ladders or vines or cobwebs (they slow us down)
	 *
	...
	static boolean fullyPassable(CalculationContext context, int x, int y, int z) {
		return fullyPassable(context.bsi.world, context.bsi.isPassableBlockPos.set(x, y, z), context.bsi.get0(x, y, z));
	}
	...
	static boolean isBlockNormalCube(BlockState state) {
		Block block = state.getBlock();
		if (block instanceof BambooBlock || block instanceof PistonExtensionBlock || block instanceof ScaffoldingBlock || block instanceof ShulkerBoxBlock)
			return false;
	...
	static boolean isReplaceable(int x, int y, int z, BlockState state, BlockStateInterface bsi) {
		...
		/*
		 * public boolean isReplaceable(IBlockAccess worldIn, BlockPos pos) { return ((Integer)worldIn.getBlockState(pos).getValue(LAYERS)).intValue() == 1; }
		 */
		Block block = state.getBlock();
		if (block instanceof AirBlock)
	...
	/**
	 * Returns whether or not the specified block is water, regardless of whether or not it is flowing.
	 *
	...
	/**
	 * Returns whether or not the block at the specified pos is water, regardless of whether or not it is flowing.
	 *
	 * @param ctx The player context
	...
	static void moveTowards(IPlayerContext ctx, MovementState state, BlockPos pos) {
		state.setTarget(new MovementTarget(new baritone.api.utils.Rotation(BRotationUtils.calcRotationFromVec3d(ctx.playerHead(), VecUtils.getBlockPosCenter(pos), ctx.playerRotations()).getYaw(), ctx.player().pitch), false)).setInput(Input.MOVE_FORWARD, true);
	}

	static boolean possiblyFlowing(BlockState state) {
		FluidState fluidState = state.getFluidState();
		return fluidState.getFluid() instanceof BaseFluid && fluidState.getFluid().getLevel(fluidState) != 8;
	}
