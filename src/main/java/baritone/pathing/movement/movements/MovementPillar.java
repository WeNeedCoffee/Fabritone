...
public class MovementPillar extends Movement {
	...
	public MovementState updateState(MovementState state) {
		...
		Rotation rotation = BRotationUtils.calcRotationFromVec3d(ctx.player().getCameraPosVec(1.0F), VecUtils.getBlockPosCenter(positionToPlace), new Rotation(ctx.player().yaw, ctx.player().pitch));
		if (!ladder) {
		...
		if (ladder) {
			...
			/*
			 * if (thePlayer.getPosition0().getX() != from.getX() || thePlayer.getPosition0().getZ() != from.getZ()) { Baritone.moveTowardsBlock(from); }
			 */
			...
			if (!blockIsThere) {
				...
				if (!(fr instanceof AirBlock || frState.getMaterial().isReplaceable())) {
					BRotationUtils.reachable(ctx.player(), src, ctx.playerController().getBlockReachDistance()).map(rot -> new MovementState.MovementTarget(rot, true)).ifPresent(state::setTarget);
