...
public class MovementDescend extends Movement {
	...
	public MovementState updateState(MovementState state) {
		...
		if ((playerFeet.equals(dest) || playerFeet.equals(fakeDest)) && (MovementHelper.isLiquid(ctx, dest) || ctx.player().getY() - dest.getY() < 0.5))
			// Wait until we're actually on the ground before saying we're done because sometimes we continue to fall if the next action starts immediately
			return state.setStatus(MovementStatus.SUCCESS);
		/*
		 * else { // System.out.println(player().posY + " " + playerFeet.getY() + " " + (player().posY - playerFeet.getY())); }
		 */
		if (safeMode()) {
			double destX = (src.getX() + 0.5) * 0.17 + (dest.getX() + 0.5) * 0.83;
			double destZ = (src.getZ() + 0.5) * 0.17 + (dest.getZ() + 0.5) * 0.83;
			ClientPlayerEntity player = ctx.player();
			state.setTarget(new MovementState.MovementTarget(new Rotation(BRotationUtils.calcRotationFromVec3d(player.getCameraPosVec(1.0F), new Vec3d(destX, dest.getY(), destZ), new Rotation(player.yaw, player.pitch)).getYaw(), player.pitch), false)).setInput(Input.MOVE_FORWARD, true);
			return state;
		}
