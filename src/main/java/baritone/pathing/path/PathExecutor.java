...
public class PathExecutor implements IPathExecutor, Helper {
	...
	/**
	 * Default value is equal to 10 seconds. It's find to decrease it, but it must be at least 5.5s (110 ticks). For more information, see issue #102.
	 *
	 ...
	 * Tick this executor
	 *
	 * @return True if a movement just finished (and the player is therefore in a "stable" state, like, not sneaking out over lava), false otherwise
	 */
	public boolean onTick() {
		...
		/*
		 * long end = System.nanoTime() / 1000000L; if (end - start > 0) { System.out.println("Recalculating break and place took " + (end - start) + "ms"); }
		 */
	...
	private Pair<Vec3d, BlockPos> overrideFall(MovementFall movement) {
		...
		return new Pair<>(new Vec3d(flatDir.getX() * len + movement.getDest().x + 0.5, movement.getDest().y, flatDir.getZ() * len + movement.getDest().z + 0.5), movement.getDest().add(flatDir.getX() * (i - pathPosition), 0, flatDir.getZ() * (i - pathPosition)));
	}
	...
	private boolean shouldSprintNextTick() {
		...
		if (current instanceof MovementFall) {
			...
			if (data != null) {
				...
				clearKeys();
				behavior.baritone.getLookBehavior().updateTarget(BRotationUtils.calcRotationFromVec3d(ctx.playerHead(), data.getLeft(), ctx.playerRotations()), false);
