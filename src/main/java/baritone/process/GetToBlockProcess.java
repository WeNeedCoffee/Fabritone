...
public final class GetToBlockProcess extends BaritoneProcessHelper implements IGetToBlockProcess {
	...
	private Goal createGoal(BlockPos pos) {
		...
		if (blockOnTopMustBeRemoved(gettingTo.getBlock()) && MovementHelper.isBlockNormalCube(baritone.bsi.get0(pos.up())))
		...
		for (BlockPos pos : knownLocations) {
			...
			if (reachable.isPresent()) {
				...
				if (knownLocations.contains(ctx.getSelectedBlock().orElse(null))) {
					...
					System.out.println(ctx.player().container);
					if (!(ctx.player().container instanceof PlayerContainer))
						return true;
				}
