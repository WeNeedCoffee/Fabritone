...
public class BlockPlaceHelper implements Helper {
    ...
    public void tick(boolean rightClickRequested) {
        ...
        if (!rightClickRequested || (MinecraftClient.getInstance().player.getVehicle()  != null && MinecraftClient.getInstance().player.getVehicle() instanceof BoatEntity) || mouseOver == null || mouseOver.getType() != HitResult.Type.BLOCK) {
            return;
        }
