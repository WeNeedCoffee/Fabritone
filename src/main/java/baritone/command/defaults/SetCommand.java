	...
	public void execute(String label, IArgConsumer args) throws CommandException {
		...
		if (paginate) {
			...
			Paginator.paginate(args, new Paginator<>(toPaginate), () -> logDirect(!search.isEmpty() ? String.format("All %ssettings containing the string '%s':", viewModified ? "modified " : "", search) : String.format("All %ssettings:", viewModified ? "modified " : "")), setting -> {
				Text typeComponent = new LiteralText(String.format(" (%s)", settingTypeToString(setting)));
				typeComponent.getStyle().setColor(Formatting.DARK_GRAY);
				...
				Text component = new LiteralText(setting.getName());
		...
		if (!doingSomething && !args.hasAny()) {
			...
			Text oldValueComponent = new LiteralText(String.format("Old value: %s", oldValue));
			oldValueComponent.getStyle().setColor(Formatting.GRAY).setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new LiteralText("Click to set the setting back to this value"))).setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, FORCE_COMMAND_PREFIX + String.format("set %s %s", setting.getName(), oldValue)));
			logDirect(oldValueComponent);
			if (setting.getName().equals("chatControl") && !(Boolean) setting.value && !Baritone.settings().chatControlAnyway.value && !Baritone.settings().clientMode.value || setting.getName().equals("chatControlAnyway") && !(Boolean) setting.value && !Baritone.settings().chatControl.value) {
