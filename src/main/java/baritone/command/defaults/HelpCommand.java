...
public class HelpCommand extends Command {
	...
	public void execute(String label, IArgConsumer args) throws CommandException {
		...
		if (!args.hasAny() || args.is(Integer.class)) {
			Paginator.paginate(args, new Paginator<>(baritone.getCommandManager().getRegistry().descendingStream().filter(command -> !command.hiddenFromHelp()).collect(Collectors.toList())), () -> logDirect("All Baritone commands (clickable):"), command -> {
			...
			ICommand command = baritone.getCommandManager().getCommand(commandName);
			if (command == null)
	...
	public Stream<String> tabComplete(String label, IArgConsumer args) throws CommandException {
		if (args.hasExactlyOne())
			return new TabCompleteHelper().addCommands(baritone.getCommandManager()).filterPrefix(args.getString()).stream();
		return Stream.empty();
	}
}
