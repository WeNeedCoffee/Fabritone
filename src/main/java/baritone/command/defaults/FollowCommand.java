...
public class FollowCommand extends Command {
	...
	private enum FollowGroup {
		ENTITIES(LivingEntity.class::isInstance), PLAYERS(PlayerEntity.class::isInstance); /*
																							 * , FRIENDLY(entity -> entity.getAttackTarget() != HELPER.mc.player), HOSTILE(FRIENDLY.filter.negate());
																							 */
	...
	public void execute(String label, IArgConsumer args) throws CommandException {
		...
		if (group != null) {
			...
			if (classes.isEmpty()) {
				...
				classes.stream().map(Registry.ENTITY_TYPE::getId).map(Objects::requireNonNull).map(Identifier::toString).forEach(this::logDirect);
			}
		}
	}
