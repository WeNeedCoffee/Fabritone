...
public class BaritoneChatControl implements Helper, AbstractGameEventListener {
	...
	private void logRanCommand(String command, String rest) {
		if (settings.echoCommands.value) {
			...
			component.getStyle().setColor(Formatting.WHITE).setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new LiteralText("Click to rerun command"))).setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, FORCE_COMMAND_PREFIX + msg));
			logDirect(component);
		}
	}
	...
	public boolean runCommand(String msg) {
		...
		ArgConsumer argc = new ArgConsumer(manager, pair.getRight());
		if (!argc.hasAny()) {
			Settings.Setting<?> setting = settings.byLowerName.get(command.toLowerCase(Locale.US));
			if (setting != null) {
				logRanCommand(command, rest);
				if (setting.getValueClass() == Boolean.class) {
					manager.execute(String.format("set toggle %s", setting.getName()));
				} else {
					manager.execute(String.format("set %s", setting.getName()));
				}
				return true;
			}
			...
			for (Settings.Setting<?> setting : settings.allSettings) {
				...
				if (setting.getName().equalsIgnoreCase(pair.getLeft())) {
					logRanCommand(command, rest);
					try {
						manager.execute(String.format("set %s %s", setting.getName(), argc.getString()));
		...
		if (manager.getCommand(pair.getLeft()) != null) {
			logRanCommand(command, rest);
		}

		return manager.execute(pair);
	}

	public Stream<String> tabComplete(String msg) {
		try {
			List<ICommandArgument> args = CommandArguments.from(msg, true);
			ArgConsumer argc = new ArgConsumer(manager, args);
			if (argc.hasAtMost(2)) {
				if (argc.hasExactly(1))
					return new TabCompleteHelper().addCommands(manager).addSettings().filterPrefix(argc.getString()).stream();
