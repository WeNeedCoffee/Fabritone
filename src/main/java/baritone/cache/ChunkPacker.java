...
public final class ChunkPacker {
	...
	private static PathingBlockType getPathingBlockType(BlockState state, WorldChunk chunk, int x, int y, int z) {
		...
		if (MovementHelper.isWater(state)) {
			...
			if (x != 15 && MovementHelper.possiblyFlowing(getFromChunk(chunk, x + 1, y, z)) || x != 0 && MovementHelper.possiblyFlowing(getFromChunk(chunk, x - 1, y, z)) || z != 15 && MovementHelper.possiblyFlowing(getFromChunk(chunk, x, y, z + 1)) || z != 0 && MovementHelper.possiblyFlowing(getFromChunk(chunk, x, y, z - 1)))
				return PathingBlockType.AVOID;
